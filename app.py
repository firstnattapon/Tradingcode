...
class Run_model :
    ...
    def inverse (self,upper=100.0 ,lowwer=0.0 , Asset_prices=0):
        ...
        st.write('Cash_total = ' , round(Asset_prices * abs(slope) , 4)  , '({})'.format(round(self.capital/2 , 4)) )
        st.write('Port_total = ' , round((Asset_prices * abs(slope)) + round(self.capital/2,4),4))
...
if __name__ == '__main__':
    ...
    if  st.checkbox('Inverse (ผกผัน)', value= 1):
        st.sidebar.text('-'*40)
        capital_inverse  = st.sidebar.number_input('capital_inverse(เงินทุนเริ่มต้น)',min_value=0.0,max_value=20000.0,value=43.1 ,step=0.1,format='%f')   
    ...
    if  st.checkbox('Direct (ผันตรง)', value=0 ):
        st.sidebar.text('-'*40)
        capital_Direct   = st.sidebar.number_input('capital_Direct(เงินทุนเริ่มต้น) ',min_value=0.0,max_value=20000.0,value=43.1,step=0.1,format='%f')
